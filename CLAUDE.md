...
**Invoice Web**은 Notion에서 작성한 견적서를 클라이언트가 웹 링크로 확인하고 PDF로 저장할 수 있는 프리랜서용 견적서 공유 서비스입니다.
...
├── globals.css             # TailwindCSS v4 스타일 + 색상 토큰 + 인쇄 스타일
├── providers.tsx           # next-themes ThemeProvider (use client)
└── invoice/
    └── [id]/
        └── page.tsx        # 견적서 상세 (공개 접근, 서버 컴포넌트)

components/
├── layout/                 # Header, Footer 레이아웃 컴포넌트
├── invoice/                # 견적서 관련 컴포넌트
│   ├── invoice-list.tsx    # 견적서 목록 카드 + 링크 복사 버튼
│   └── invoice-detail.tsx  # 견적서 상세 뷰 + PDF 저장 버튼
└── ui/                     # shadcn/ui 컴포넌트들
